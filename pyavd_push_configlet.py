...
if __name__ == "__main__":
    ...
    device_inventory =  [
        {
            "hostname": "mhd367-spine1",
            "ip": "172.28.140.155",
            "serial_number": "JPN2439P0XU",
            "system_mac_address": "9c69.ed71.68c1"
        }
    ]

    # Create a list of CVEosConfig objects that contain the path to the config files and the device information
    configlets = [
        CVEosConfig(file=f"/workspaces/vu-avd/POC/intended/configs/{host['hostname']}.cfg", device=CVDevice(hostname=f"{host['hostname']}", serial_number=f"{host['serial_number']}", system_mac_address=f"{host['system_mac_address']}", _exists_on_cv=True))
